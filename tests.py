...
def printf(name, test, status):
    print(name, end="")
    print(str(test).rjust(7 - len(name), " "), "|", status)
...
def check_req(r, test_id, status) -> bool:
    if r.status_code == status:
        printf("TEST", test_id, f"ok     {r.status_code}: {r.text}")
    else:
        printf("TEST", test_id, f"failed {r.status_code}: {r.text}")
    return r.status_code == status

def test(test_id, caption, sequent):
    testf(caption)
    SQL_init_schema(cur)
    ...
    for req in sequent:
        ...
        if not check_req(r, test_id, req["result"]): break
